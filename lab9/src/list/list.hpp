...
public:
    ...
    Iterator<T> first() const { return Iterator<T>(head); }
    Iterator<T> last() const { return Iterator<T>(nullptr); } // зачем оно вообще в списке

    /// err 2 == 3
    Node<T> *operator[](int index)
    {
        if (index < 0 || index >= size)
            throw logic_error("index < 0 or >= list size"); // type 2
        Node<T> *node = head;
...
{
    if (size == LIST_MAX_SIZE)
        throw logic_error("List size more than LIST_MAX_SIZE!"); // type 2
